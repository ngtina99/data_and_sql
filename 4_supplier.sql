...
SELECT JNAME, CITY
FROM PROJECT 
WHERE J_ID IN (SELECT J_ID FROM SUPPLY WHERE S_ID 
IN
...
SELECT JNAME, CITY FROM PROJECT WHERE J_ID NOT IN  (SELECT J_ID FROM SUPPLY WHERE S_ID 
IN  (SELECT S_ID FROM SUPPLIER WHERE  CITY = 'LONDON'))
...
FROM SUPPLY SY
JOIN SUPPLIER SUP ON SY.S_ID = SUP.S_ID
JOIN PART P ON P.P_ID = SY.P_ID
JOIN PROJECT PJ ON SY.J_ID = PJ.J_ID
WHERE SUP.CITY = P.CITY AND P.CITY = PJ.CITY